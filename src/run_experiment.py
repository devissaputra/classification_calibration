...
EXPECTED_DATA_SHA256 = "d1513ec63b385506f7cfce9f2c5caa9fe99e7ba4e8c3fa264b3aaf0f849ed32d"
...
def validate_dataset_hash(csv_bytes: bytes, expected_sha256: str = EXPECTED_DATA_SHA256) -> str:
    actual = hashlib.sha256(csv_bytes).hexdigest()
    if actual != expected_sha256:
        raise ValueError(
            f"Unexpected bank-full.csv SHA-256: {actual}; expected {expected_sha256}. "
            "The frozen research protocol requires the exact validated UCI source bytes."
        )
    return actual
...
def load_real_data(data_path: str | Path | None = None, cache_dir: str | Path = "data/cache"):
    ...
    data_sha256 = validate_dataset_hash(csv_bytes)
...
def build_results_latex(dataset: dict, summary: dict, deltas: dict) -> str:
    bs = "\\"
    row_end = bs + bs
    labels = {
        "dummy_prior": "Class-prior baseline",
        "logistic_uncalibrated": "Logistic, uncalibrated",
        "logistic_sigmoid": "Logistic + sigmoid",
        "logistic_isotonic": "Logistic + isotonic",
    }
    lines = [
        ...
        "",
        f"Dataset: UCI Bank Marketing (dataset {dataset['uci_id']}), "
        f"$n={dataset['n_samples']:,}$, {dataset['n_features']} predictors, "
        f"positive prevalence {dataset['positive_rate']:.4f}.",
        "",
        ...
        f"{bs}toprule",
        "Model & ROC-AUC & AP & Brier $\\downarrow$ & Log loss $\\downarrow$ & ECE-10 $\\downarrow$ " + row_end,
        f"{bs}midrule",
    ]
    for model, metrics in summary.items():
        lines.append(
            f"{labels.get(model, model)} & "
            f"{metrics['roc_auc']['mean']:.4f} & "
            f"{metrics['average_precision']['mean']:.4f} & "
            f"{metrics['brier']['mean']:.4f} & "
            f"{metrics['log_loss']['mean']:.4f} & "
            f"{metrics['ece_10']['mean']:.4f} " + row_end
        )
    iso = deltas["logistic_isotonic"]
    sig = deltas["logistic_sigmoid"]
    lines += [
        f"{bs}bottomrule",
        f"{bs}end{{tabular}}",
        f"{bs}caption{{Mean performance across five fixed stratified 80/20 holdouts. Full standard deviations, sensitivity analyses, and error diagnostics are stored in the machine-readable results.}}",
        f"{bs}label{{tab:main-results}}",
        f"{bs}end{{table}}",
        "",
        f"{bs}subsection{{Paired robustness summaries}}",
        (
            "Across the five fixed splits, isotonic calibration changed Brier score by "
            f"{iso['brier']['mean_delta']:.4f} on average and log loss by "
            f"{iso['log_loss']['mean_delta']:.4f} relative to uncalibrated logistic regression. "
            "These are descriptive robustness summaries rather than independent-replication inference."
        ),
        (
            "Sigmoid calibration produced a much smaller mean Brier change of "
            f"{sig['brier']['mean_delta']:.4f}. Exact paired bootstrap intervals are recorded in "
            "\\texttt{results/metrics.json} and \\texttt{paper/results.md}."
        ),
        "",
        f"{bs}subsection{{Operational feature ablation}}",
        ...
        "",
        f"{bs}paragraph{{Data identity.}}",
        f"The exact validated \\texttt{{bank-full.csv}} SHA-256 is \\texttt{{{dataset['sha256']}}}.",
        "",
    ]
    return "\n".join(lines)
...
def run_experiment(results_dir: str | Path = "results", data_path: str | Path | None = None, quick: bool = False):
    ...
    Path("paper/results.tex").write_text(
        build_results_latex(dataset, repeated_summary, deltas),
        encoding="utf-8",
    )
    return results
